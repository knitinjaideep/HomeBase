-- HomeScope — extend import_household_backup() to cover HomeBase
--
-- "ownedHome"/"maintenanceItems"/"repairProjects" (0020) are now part of the
-- backup envelope (see src/lib/backup.ts's backupSchema/CLOUD_TABLES), so a
-- JSON restore must also replace/import them — otherwise exporting a backup
-- would capture them but restoring one would silently drop them. Exact same
-- shape as 0018_notes_backup_function.sql: `create or replace function` on
-- the current definition, additive only — three new `delete` lines and
-- three new entries in the returned counts. Everything else is
...
begin
  ...
  delete from "repairProjects" where "householdId" = p_household_id;
  delete from "maintenanceItems" where "householdId" = p_household_id;
  delete from "ownedHome" where "householdId" = p_household_id;
  ...
  counts := jsonb_build_object(
    ...
    'ownedHome', private.import_household_table('ownedHome', p_household_id, p_data->'ownedHome'),
    'maintenanceItems', private.import_household_table('maintenanceItems', p_household_id, p_data->'maintenanceItems'),
    'repairProjects', private.import_household_table('repairProjects', p_household_id, p_data->'repairProjects')
  );
